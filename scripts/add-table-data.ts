...
// Основная функция
async function addTableDataToFirestore() {
  try {
    // Замените на реальный userId (можно получить из аутентификации)
    const userId = 'your-user-id' // Здесь нужно указать реальный ID пользователя
    const projectId = 'your-project-id' // Здесь нужно указать реальный ID проекта
    ...
    console.log(`ID компании "Грин": ${greenCompanyId}`)
    console.log(`ID компании "БлэкБокс": ${blackBoxCompanyId}`)
    ...
    for (const person of tableData) {
      ...
      console.log(`Добавлен запрос для ${person.name} с ID: ${docRef.id}`)
    }
