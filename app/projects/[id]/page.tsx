...
export default function ProjectPage() {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-[#2D3748] flex flex-col">
      {/* Mobile Header */}
      <motion.div
        initial={{ opacity: 0, filter: "blur(20px)" }}
        animate={{
        ...
        className="lg:hidden w-full bg-[#1A202C] border-b border-[#4A5568] p-4"
      >
        <div className="flex items-center gap-3 mb-3">
          <button
            onClick={handleBack}
            className="flex items-center gap-2 text-[#CBD5E0] transition-colors hover:text-[#E5E7EB] active:bg-[#374151] p-2 rounded-lg"
          >
            <ArrowLeft className="h-5 w-5" />
            <span className="text-sm font-inter">Назад</span>
          </button>
        </div>
        <div>
          <h1 className="text-lg font-light text-[#E5E7EB] font-montserrat">{project.name}</h1>
      ...
      <div className="flex flex-col lg:flex-row flex-1">
        ...
        <motion.div
          ...
          transition={{ duration: 0.5, ease: "easeOut" }}
          className="w-full lg:w-80 bg-[#1A202C] border-b lg:border-r lg:border-b-0 border-[#4A5568] flex flex-col"
        >
          {/* Desktop Header */}
          <div className="hidden lg:block p-6 border-b border-[#4A5568]">
            <div className="flex items-center gap-4 mb-4">
              ...
              >
                <ArrowLeft className="h-5 w-5" />
                <span className="text-base font-inter">Назад к проектам</span>
              </button>
            </div>
            <div>
          ...
          <div className="p-4 lg:p-6 flex-1">
            ...
            <div className="lg:hidden">
              <div className="flex gap-3 overflow-x-auto pb-2 mb-4">
                <motion.button
                  ...
                  className="flex-shrink-0 flex items-center gap-2 rounded-xl bg-[#3B82F6] px-4 py-3 text-white transition-all hover:bg-[#2563EB] text-sm min-w-[120px] justify-center"
                  whileTap={{ scale: 0.95 }}
                >
                  <Plus className="h-4 w-4" />
                  <span className="font-inter">Добавить</span>
                </motion.button>

                <motion.button
                  onClick={() => setIsEnhancedStatsOpen(true)}
                  className="flex-shrink-0 flex items-center gap-2 rounded-xl bg-[#4A5568] px-4 py-3 text-[#E5E7EB] transition-all hover:bg-[#374151] text-sm min-w-[120px] justify-center"
                  whileTap={{ scale: 0.95 }}
                >
              ...
              <div className="grid grid-cols-3 gap-3">
                <motion.button
                  ...
                  className="flex flex-col items-center justify-center rounded-xl bg-[#4A5568] p-3 text-[#E5E7EB] transition-all hover:bg-[#374151] relative min-h-[80px]"
                  whileTap={{ scale: 0.95 }}
                >
                  <Check className="h-5 w-5 mb-1" />
                  <span className="font-inter text-xs text-center">Принято</span>
                  <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#10B981] text-xs font-medium text-white">
                ...
                >
                  ...
                  <span className="font-inter text-xs text-center">Отказ</span>
                  <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#EF4444] text-xs font-medium text-white">
                ...
                >
                  ...
                  <span className="font-inter text-xs text-center">Не отв</span>
                  {unansweredRequests.length > 0 && (
                    <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#F59E0B] text-xs font-medium text-white">
            ...
            <div className="hidden lg:block space-y-4">
              <motion.button
                onClick={() => setIsManualRequestOpen(true)}
                className="flex items-center gap-3 rounded-lg bg-[#3B82F6] px-4 py-3 text-white transition-all hover:bg-[#2563EB] text-base w-full"
                whileTap={{ scale: 0.98 }}
              >
              ...
              <motion.button
                onClick={() => setIsEnhancedStatsOpen(true)}
                className="flex items-center gap-3 rounded-lg bg-[#4A5568] px-4 py-3 text-[#E5E7EB] transition-all hover:bg-[#374151] text-base w-full"
                whileTap={{ scale: 0.98 }}
              >
              ...
              <motion.button
                onClick={() => handleOpenPanel("accepted")}
                className="flex items-center justify-between rounded-lg bg-[#4A5568] px-4 py-3 text-[#E5E7EB] transition-all hover:bg-[#374151] text-base w-full"
                whileTap={{ scale: 0.98 }}
              >
          ...
          {statistics && (
            <div className="p-4 lg:p-6 border-t border-[#4A5568]">
              <motion.button
                onClick={() => setIsEnhancedStatsOpen(true)}
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className="text-sm sm:text-base lg:text-lg font-semibold text-[#E5E7EB] font-inter">Быстрая статистика</h3>
                </div>

                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                  <div className="bg-[#374151] rounded-lg p-2 sm:p-3">
                    <div className="text-sm sm:text-base lg:text-lg font-bold text-[#E5E7EB] font-inter">{statistics.total?.all || 0}</div>
                    <div className="text-xs text-[#9CA3AF] font-inter">Всего</div>
                  </div>

                  <div className="bg-[#374151] rounded-lg p-2 sm:p-3">
                    <div className="text-sm sm:text-base lg:text-lg font-bold text-[#E5E7EB] font-inter">{statistics.today?.count || 0}</div>
                    <div className="text-xs text-[#9CA3AF] font-inter">Сегодня</div>
                  </div>

                  <div className="bg-[#374151] rounded-lg p-2 sm:p-3">
                    <div className="text-sm sm:text-base lg:text-lg font-bold text-[#10B981] font-inter">{statistics.total?.accepted || 0}</div>
                    <div className="text-xs text-[#9CA3AF] font-inter">Принято</div>
                  </div>

                  <div className="bg-[#374151] rounded-lg p-2 sm:p-3">
                    <div className="text-sm sm:text-base lg:text-lg font-bold text-[#EF4444] font-inter">{statistics.total?.rejected || 0}</div>
          ...
          <div className="p-4 sm:p-6 border-b border-[#4A5568]">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-[#E5E7EB] font-inter">Заявки</h3>
